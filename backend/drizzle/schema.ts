import { pgTable, pgSchema, index, foreignKey, uuid, text, timestamp, uniqueIndex, inet, unique, jsonb, bigserial, varchar, boolean, json, smallint, integer, serial, primaryKey } from "drizzle-orm/pg-core"
...
export const userVocabRecord = pgTable("user_vocab_record", {
	userId: uuid("user_id").notNull(),
	vocabulary: varchar("vocabulary", { length: 32 }).notNull(),
	acquainted: boolean("acquainted"),
	...
	id: integer("id").primaryKey().generatedByDefaultAsIdentity({ name: "user_vocab_record_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
},
(table) => {
	return {
...
export const vocabularyList = pgTable("vocabulary_list", {
	...
	original: boolean("original").default(true),
