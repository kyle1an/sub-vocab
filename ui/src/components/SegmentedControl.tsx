      ...
      <div class="grid w-full select-none auto-cols-[1fr] grid-flow-col overflow-hidden rounded-[9px] bg-[#EFEFF0] p-0.5 outline-none">
        ...
        {props.segments.map((item, index) => (
          ...
          >
            ...
            >
              <span class="relative z-10 flex justify-center text-sm/6 text-black transition-all duration-200 ease-[ease] will-change-transform group-hover:opacity-20 group-focus:opacity-20 group-active:opacity-20 group-[&:checked+label]/i:opacity-100 group-active:group-[&:not(:checked)+label]/i:scale-95">
                {item.label}
              </span>
            </label>
          </div>
